...
object Feedback {
    ...
    val route = out@{ exchange: HttpExchange ->
        ...
        log(LogLevel.INFO, "Request #$hash /feedback <- $ipAddress, api version=$reqApiVersion, data=$reqString")

        if (exchange.returnIfApiVersionInsufficient()) {
            log(LogLevel.INFO, "Request #$hash /feedback -> $ipAddress, api version insufficient")
            return@out
        }
        ...
        try {
            ...
            log(LogLevel.INFO, "Request #$hash /feedback :: Can't parse request")
            statusCode = 400
        } catch (e: Exception) {
            log(LogLevel.ERROR, "Request #$hash /feedback :: Unknown error: ${e.message}", e)
            statusCode = 500
        }

        log(LogLevel.INFO, "Request #$hash /feedback -> $ipAddress, api version=$reqApiVersion, status=$statusCode")
