...
object CourseListParserV12 {
    ...
    private fun parseExtraMark(json: JSONObject) =
        ExtraMark(json["name"] as String, OverallMark(json["mark"] as Double))

    private fun parseExtraMarks(json: JSONArray) = ExtraMarks().apply {
        json.forEach { obj ->
            add(parseExtraMark(obj as JSONObject))
        }
    }
    ...
    private fun parseCourse(json: JSONObject) = Course().apply {
        ...
        extraMarks = (json["extra_marks"] as JSONArray?)?.let { parseExtraMarks(it) }
