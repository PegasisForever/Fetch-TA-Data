...
object CourseListParserV12 {
    ...
    private fun parseSmallMarkGroup(json: Map<*,*>) = SmallMarkGroup().apply {
        (json["smallmarks"] as List<*>).forEach { smallMarkJSON ->
            add(parseSmallMark(smallMarkJSON as Map<*,*>))
        }
    }
    ...
    private fun parseExtraMark(json: Map<*,*>) =
        ExtraMark(json["name"] as String, OverallMark(json["mark"] as Double))

    private fun parseExtraMarks(json: List<*>) = ExtraMarks().apply {
        json.forEach { obj ->
            add(parseExtraMark(obj as Map<*,*>))
        }
    }
    ...
    private fun parseCourse(json: Map<*,*>) = Course().apply {
        ...
        extraMarks = (json["extra_marks"] as List<*>?)?.let { parseExtraMarks(it) }
        noCredit = json["no_credit"] as Boolean?
