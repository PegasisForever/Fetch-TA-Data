...
object CourseListSerializerV12 {
    ...
    private fun serializeExtraMark(extraMark: ExtraMark): JSONObject {
        val obj = JSONObject()
        obj["name"] = extraMark.name
        obj["mark"] = extraMark.mark.getMarkValue()
        return obj
    }

    private fun serializeExtraMarkList(extraMarks: ExtraMarks): JSONArray {
        val array = JSONArray()
        extraMarks.forEach { extraMark ->
            array.add(serializeExtraMark(extraMark))
        }
        return array
    }
    ...
    private fun serializeCourse(course: Course): JSONObject {
        ...
        obj["extra_marks"] = course.extraMarks?.let { serializeExtraMarkList(it) }
        obj["no_credit"] = course.noCredit
